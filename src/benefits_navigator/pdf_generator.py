...
# The Helvetica metrics and the shrink-to-fit logic live in ``formmap.textfit``
# now that the mapping layer needs them too: an overlay renderer that measured
# differently from this one would disagree with it about whether a value fits,
# on the same value, in the same size box. Re-bound to the private names this
# module and its tests already use, so the move changed no behaviour here.
from .formmap.textfit import (
    FIELD_PADDING as _FIELD_PADDING,
    FIELD_PADDING_Y as _FIELD_PADDING_Y,
    HELVETICA_WIDTHS as _HELVETICA_WIDTHS,
    LINE_HEIGHT as _LINE_HEIGHT,
    MIN_FONT_SIZE as _MIN_FONT_SIZE,
    fits as _fits,
    helvetica_width as _helvetica_width,
    largest_size_that_fits as _largest_size_that_fits,
    wrap_to_width as _wrap_to_width,
)

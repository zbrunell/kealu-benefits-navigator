...
vi.mock("next/headers", () => {
  ...
  return {
    cookies: vi.fn(() =>
      Promise.resolve({
        get: (name: string) =>
          _cookies[name] ? { name, value: _cookies[name] } : undefined,
        ...
        __setTestCookie: (name: string, value: string) => {
          _cookies[name] = value;
        },
        __clearCookies: () => {
          _cookies = {};
...
describe("GET /api/session", () => {
  ...
  it("expired session is treated as new (new Set-Cookie issued)", async () => {
    ...
    const store = await getCookieStore();
    (store as any).__setTestCookie(
      "session",
      "expired-session-id-that-does-not-exist",
    );

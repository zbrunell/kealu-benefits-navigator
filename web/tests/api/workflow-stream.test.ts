...
vi.mock("next/headers", () => ({
  cookies: vi.fn(() =>
    Promise.resolve({
      get: (name: string) =>
        _sessionCookies[name]
          ? { name, value: _sessionCookies[name] }
          : undefined,
...
// Deterministic session store mock
...
function makeStreamRequest(runId: string, sessionId?: string): Request {
  ...
  return new Request(`http://localhost/api/workflow/${runId}/stream`, {
    headers,
  });
}
...
describe("GET /api/workflow/[runId]/stream", () => {
  ...
  it("returns HTTP 403 when session.runId does not match path runId", async () => {
    // The session store is seeded with TEST_SESSION_ID -> { runId: TEST_RUN_ID }
    // The request will use the session for TEST_SESSION_ID, but request a different runId
    _sessionCookies["session"] = TEST_SESSION_ID;
    const req = makeStreamRequest("different-run-id-9999", TEST_SESSION_ID);
    const res = await GET(req, {
      params: Promise.resolve({ runId: "different-run-id-9999" }),
    });
    expect(res.status).toBe(403);
  });
